...
--Uso dell'outer join
--1
select corsi.id, corsi.denominazione, Professori.cognome, professori.nome
from Corsi full outer join Professori on corsi.professore=Professori.id
order by Corsi.denominazione;

--2
select  Professori.id, Professori.cognome,studenti.cognome,studenti.nome,studenti.matricola
from Studenti full outer join Professori on Studenti.relatore=Professori.id
where studenti.relatore!=NULL
order by Professori.id;

--3
select Studenti.cognome, Studenti.nome, Studenti.relatore
from Studenti full outer join professori
on studenti.relatore=professori.id
where corsodilaurea=1
order by studenti.cognome;
...
--2 cognome,  nome  e  qualifica  (’studente’/’professore’)  degli  studenti  e  professori  dell’Universit`a  diGenova.2
select studenti.cognome, studenti.nome, professori.cognome, professori.nome
from Studenti full outer join professori 
on studenti.relatore=professori.id;

--3 Domanda:  Il numero di tuple delle precedenti due interrogazioni `e lo stesso?  Se no, perch ́e?

--4 
select professori.cognome,professori.nome
from Studenti full outer join professori on studenti.relatore=professori.id
where studenti.cognome=professori.cognome and studenti.nome=professori.nome;
...
--6  
select studenti.matricola, studenti.cognome, studenti.nome
from Studenti full outer join esami on studenti.matricola=esami.studente
where studenti.corsodilaurea=9 and esami.corso='bdd1n' and esami.corso!='ig' and esami.data>='6/01/2010' and esami.data<='6/30/2010'
