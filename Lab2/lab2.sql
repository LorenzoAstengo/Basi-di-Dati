...
UNIQUE(Facolta, Denominazione)OT LIKE 'L%';
);
...
from Studenti,PianiDiStudio
where Anno='5' and annoaccademico='2012' and CorsoDiLaurea='9' and Studenti.relatore is not null
...
INSERT INTO Studenti VALUES
('dy3192', 'Farazza', 'Michele', 'Genova', '07/03/1982',select distinct Studenti.Cognome, Studenti.nome
from Studenti,PianiDiStudio
where Anno='5' and annoaccademico='2012' and CorsoDiLaurea='9' and Studenti.relatore is not null
order by Studenti.Cognome desc; 'Firenze', 9, '2010', null, null);
INSERT INTO Studenti VALUES
...
INSERT INTO PianiDiStudio VALUES ('h8u763d', '2010', 2); hanno presentato 
...
INSERT INTO Esami VALUES('23glot5', 'ldp', '7/9/2011', 7);AnnoAccademico integer not null,

INSERT INTO Esami VALUES('23glot5', 'r1', '01/27/2009', 9);
INSERT INTO Esami VALUES('23glot5', 'r2', '02/21/2011', 10 hanno presentato );
...
INSERT INTO Esami VALUES('dy3192fr', 'an2mate', '04/21/201 hanno presentato 0', 27);
...
from CorsiDiLaurea
where Attivazione<'2006/2007' AND Attivazione>'2009/2010'
ORDER BY Facolta, Denominazione;

--5 
SELECT Matricola, Nome, Cognome
FROM Studenti
where Relatore=null AND Iscrizione<'2007';

--6
SELECT Studente
FROM Esami
where Data>'02/02/2009';

--7
SELECT ID
FROM Corsi
where Attivato=true And Denominazione NOT LIKE 'L%';

--8 
SELECT ID
FROM Professori
where Nome LIKE '%te%' and stipendio>=12500 and stipendio<=16000;

--9
SELECT Matricola, Nome, Cognome
FROM Studenti
where (Residenza='La Spezia' and Residenza='Savona' and Residenza='Genova')or(Cognome!='Serra' or Cognome!='Melogno' or Cognome!='Giunchi')
order by Matricola desc;

--10 
SELECT Denominazione
FROM CorsiDiLaurea
where  Attivazione>='2004/2005' and Attivazione<='2009/2010'
order by Facolta;


--Interrogazioni di base che richiedono l'uso di piu relazioni
--1
SELECT Matricola
FROM Studenti
where Laurea<'01/11/2009' and CorsoDiLaurea='9';
...
FROM Professori join Corsi on Professori.id=Professore
where attivato=true
order by CorsoDiLaurea, denominazione;
...
--5
select Corsi.Denominazione, Corsi.id,CorsoDiLaurea, Corsi.professore, Attivato,CorsiDiLaurea.id, facolta
from Corsi join CorsiDiLaurea on CorsiDiLaurea.id=corsi.CorsoDiLaurea
where CorsoDiLaurea='9' and attivato=true  and Corsi.Denominazione like '__s%';

--6
select studenti.Matricola
from studenti join Esami on Esami.studente=Studenti.Matricola
where studenti.CorsoDiLaurea=1 and Voto>=18 and Esami.data='02/15/2012' and Esami.Corso='infogenM';

--7
select distinct Studenti.Cognome, Studenti.nome
from Studenti,PianiDiStudio
where Anno='5' and annoaccademico='2011' and CorsoDiLaurea='9' and Studenti.relatore is not null and Studenti.laurea is null
order by Studenti.Cognome desc;
